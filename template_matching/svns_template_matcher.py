#!/usr/bin/env python3
"""
SVNS Template Matcher — single brand / touchpoint logo detection.

Copied from erc_template_matcher.py (same matching pipeline).
Batch mode (--SVNSonly): processes folders whose name starts with ``svns``.

Configure placeholders below, then add templates under::

    svns_templates/<SVNS_BRAND_NAME> Logos/<SVNS_MATCH_ZONE>/<SVNS_TOUCHPOINT>/*.jpg

Usage:
    python svns_template_matcher.py <image_folder>
    python svns_template_matcher.py F:\\downloads\\batch --SVNSonly
    python svns_template_matcher.py <folder> --threshold 0.55 --annotate
"""

# ---------------------------------------------------------------------------
# SVNS — single brand only (folder: ``HSBC SVNS Logos/bottom_right/...``)
# ---------------------------------------------------------------------------
SVNS_BRAND_NAME = "HSBC SVNS"
SVNS_TOUCHPOINT = "TVGI"  # location subfolder under bottom_right
# Default templates folder name (also accepts common typo svns_tempates)
SVNS_TEMPLATES_DIR_NAMES = ("svns_templates", "svns_tempates")
SVNS_MATCH_ZONE = "bottom_right"  # SCREEN_ZONES key + template zone folder name
...
ANNOTATE_PREVIEW_THRESHOLD = 0.50
# SVNS batch folders (name starts with svns) — often temporal_median frames
DEFAULT_THRESHOLD_SVNS_ONLY = 0.55
...
ZONE_MIN_DETECTION_SIZES = {
    SVNS_MATCH_ZONE: STANDARD_ZONE_MIN_DETECTION_SIZE,
}

def _svns_brand_key() -> str:
    return SVNS_BRAND_NAME.lower()
...
# SVNS output: zone-focused, one best hit per template file, light overlap rules.
SVNS_BEST_MATCH_PER_TEMPLATE = True
SVNS_DETECTION_MIN_ZONE_OVERLAP = 0.55
# Small / faint TVGI logos (Lancia, Toyota, …)
SVNS_DETECTION_MIN_ZONE_OVERLAP_SMALL = 0.40
SVNS_JSON_MIN_BOX_PX_DEFAULT = 12
SVNS_JSON_MIN_BOX_DISPLAY_PX_DEFAULT = 6
SVNS_JSON_MIN_BOX_PX_BY_BRAND = {
    _svns_brand_key(): 8,
}
SVNS_JSON_MIN_BOX_DISPLAY_BY_BRAND = {
    _svns_brand_key(): 6,
}
SVNS_JSON_MIN_ZONE_OVERLAP = 0.55
SVNS_JSON_MIN_ZONE_OVERLAP_SMALL = 0.40
...
SVNS_MIN_CSV_PERCENT = 0.10
SVNS_MIN_CSV_PERCENT_DIFFICULT = 0.015
SVNS_MIN_CSV_PERCENT_BY_BRAND = {
    _svns_brand_key(): 0.02,
}

# Drop JSON annotations below this template-match confidence
SVNS_MIN_ANNOTATION_CONFIDENCE = 0.10

# Brands / zones actually loaded from svns_templates/ (set in main)
LOADED_TEMPLATE_BRANDS: Optional[frozenset] = None
LOADED_TEMPLATE_ZONES: Optional[frozenset] = None

# HSBC SVNS only — extra scales + slightly lower match threshold
DIFFICULT_MATCH_BRANDS = frozenset({_svns_brand_key()})
SMALL_MATCH_BRANDS = frozenset({_svns_brand_key()})
BRAND_MATCH_THRESHOLD_ADJUST = {
    _svns_brand_key(): -0.12,
}
SVNS_RETRY_THRESHOLD = 0.40
SVNS_PRIMARY_ZONE = SVNS_MATCH_ZONE
STANDARD_LOGO_ZONES = frozenset({SVNS_MATCH_ZONE})
STANDARD_ZONE_PREFER_MARGIN = 0.12
SMALL_BRAND_ZONE_PAD_RATIO = 0.08

# Unused in single-brand SVNS mode (kept for shared helper signatures)
...
LANCIA_RETRY_THRESHOLD = SVNS_RETRY_THRESHOLD

# Screen zones — folder name under svns_templates must match the key (SVNS_MATCH_ZONE).
SCREEN_ZONES = {
    SVNS_MATCH_ZONE: dict(SVNS_ZONE_COORDS),
}

SVNS_ALL_ZONES: Tuple[str, ...] = (SVNS_MATCH_ZONE,)
SVNS_ACTIVE_ZONES: Tuple[str, ...] = SVNS_ALL_ZONES

# Legacy --region-only: restrict to the SVNS touchpoint zone
BOTTOM_LEFT_REGION = SCREEN_ZONES[SVNS_MATCH_ZONE]


def resolve_svns_templates_dir(script_dir: Path, explicit: Optional[Path] = None) -> Path:
    """Pick templates folder: explicit path, then any ``svns_temp*`` dir with HSBC SVNS Logos."""
    if explicit is not None:
        return explicit.resolve()

    def _has_hsbc_logos(folder: Path) -> bool:
        return (folder / "HSBC SVNS Logos").is_dir()

    # Prefer svns_templates, then any svns_temp* that actually contains templates
    candidates: List[Path] = []
    for name in SVNS_TEMPLATES_DIR_NAMES:
        p = script_dir / name
        if p.is_dir():
            candidates.append(p)
    for p in sorted(script_dir.iterdir()):
        if p.is_dir() and p.name.lower().startswith("svns_temp") and p not in candidates:
            candidates.append(p)

    for folder in candidates:
        if _has_hsbc_logos(folder):
            if folder.name != SVNS_TEMPLATES_DIR_NAMES[0]:
                print(f"ℹ️  Using templates folder: {folder.name}")
            return folder

    # Empty or missing HSBC tree — still return first svns_temp* if present
    for folder in candidates:
        if folder.name != SVNS_TEMPLATES_DIR_NAMES[0]:
            print(f"ℹ️  Using templates folder: {folder.name}")
        return folder

    return script_dir / SVNS_TEMPLATES_DIR_NAMES[0]
...
def detection_center_zone(
    ...
    cx = (x1 + x2) / 2
    cy = (y1 + y2) / 2
    for zone_name in SVNS_ACTIVE_ZONES:
...
def _json_min_box_px(brand: Optional[str]) -> int:
    if brand:
        return SVNS_JSON_MIN_BOX_PX_BY_BRAND.get(brand.lower(), SVNS_JSON_MIN_BOX_PX_DEFAULT)
    return SVNS_JSON_MIN_BOX_PX_DEFAULT


def _json_min_box_display_px(brand: Optional[str]) -> int:
    if brand:
        return SVNS_JSON_MIN_BOX_DISPLAY_BY_BRAND.get(
            brand.lower(), SVNS_JSON_MIN_BOX_DISPLAY_PX_DEFAULT
        )
    return SVNS_JSON_MIN_BOX_DISPLAY_PX_DEFAULT
...
def is_valid_svns_json_detection(
    x1: int, y1: int, x2: int, y2: int, width: int, height: int,
    ...
    if box_w * scale_x < min_disp or box_h * scale_y < min_disp:
        return False
    check_zone = template_zone if template_zone in SVNS_ACTIVE_ZONES else detection_center_zone(
        x1, y1, x2, y2, width, height
    )
    if check_zone is None:
        return False
    overlap = box_overlap_fraction_in_zone(x1, y1, x2, y2, check_zone, width, height)
    min_json_overlap = SVNS_JSON_MIN_ZONE_OVERLAP
    if brand and brand.lower() in SMALL_MATCH_BRANDS:
        min_json_overlap = SVNS_JSON_MIN_ZONE_OVERLAP_SMALL
...
def _svns_detection_reject_reason(
    x1: int, y1: int, x2: int, y2: int,
    ...
    if box_overlap_fraction_in_zone(x1, y1, x2, y2, zone_name, width, height) < SVNS_DETECTION_MIN_ZONE_OVERLAP:
        return "low zone overlap"
    if not is_valid_svns_json_detection(
...
def _brand_allowed(brand: Optional[str]) -> bool:
    if not brand:
        return False
    return brand.lower() == _svns_brand_key()
...
def _output_filter_reject_reason(
    ...
    if conf < SVNS_MIN_ANNOTATION_CONFIDENCE:
        return f"low conf ({conf:.3f} < {SVNS_MIN_ANNOTATION_CONFIDENCE})"
    pct = _annotation_csv_percent(start_point, diag_point, canvas_w, canvas_h)
    b = (brand or "").lower()
    min_pct = SVNS_MIN_CSV_PERCENT_BY_BRAND.get(b, SVNS_MIN_CSV_PERCENT)
    if b in (DIFFICULT_MATCH_BRANDS | SMALL_MATCH_BRANDS):
        min_pct = min(min_pct, SVNS_MIN_CSV_PERCENT_DIFFICULT)
    if pct < min_pct:
...
def _consolidate_svns_zone(dets: List[Tuple]) -> List[Tuple]:
    """Single SVNS brand — best hit in the touchpoint zone."""
    brand_key = _svns_brand_key()
...
def consolidate_zone_detections(
    ...
    """One SVNS touchpoint zone — best hit for SVNS_BRAND_NAME."""
    by_zone: Dict[str, List[Tuple]] = {}
    for det in detections:
        zone = _detection_zone(det)
        if zone not in SVNS_ACTIVE_ZONES:
            continue
        by_zone.setdefault(zone, []).append(det)

    out: List[Tuple] = []
    for zone_name, dets in by_zone.items():
        if zone_name == SVNS_PRIMARY_ZONE:
            out.extend(_consolidate_svns_zone(dets))
...
def load_templates(templates_dir: Path) -> List[Tuple[Path, np.ndarray, str, str, Optional[str]]]:
    ...
    if brand_folders:
        ...
        for brand_folder in brand_folders:
            # Extract brand from folder name (e.g., "HSBC SVNS Logos" -> "HSBC SVNS")
            brand_name = brand_folder.name.replace(" Logos", "").replace(" logos", "").strip()
            if brand_name.lower() != _svns_brand_key():
                continue
...
def detect_svns_in_region(
    ...
    # ERC: only templates in SVNS_ACTIVE_ZONES (ignore other folders / no-zone templates)
    templates_with_zones = [
        (tp, t, b, l, z) for tp, t, b, l, z in templates
        if z is not None and z in SVNS_ACTIVE_ZONES
    ]
    templates_without_zones = [
        (tp, t, b, l, z) for tp, t, b, l, z in templates
        if z is None or z not in SVNS_ACTIVE_ZONES
    ]
    ...
    def _process_template_in_zone(
        ...
        if zone_name not in SCREEN_ZONES:
            return
        if zone_name == SVNS_PRIMARY_ZONE and (
            not brand or brand.lower() != _svns_brand_key()
        ):
            return
        ...
        if (
            not matches
            and brand
            and brand.lower() == _svns_brand_key()
        ):
            retry_thresh = max(
                SVNS_RETRY_THRESHOLD,
        ...
        if SVNS_BEST_MATCH_PER_TEMPLATE and matches:
        ...
        if debug_match:
            ...
            if matches:
                ...
                reason = _svns_detection_reject_reason(
        ...
        for match in matches:
            ...
            if det_size < zone_min_size:
                continue
            min_zone_overlap = SVNS_DETECTION_MIN_ZONE_OVERLAP
            if brand and brand.lower() in SMALL_MATCH_BRANDS:
                min_zone_overlap = SVNS_DETECTION_MIN_ZONE_OVERLAP_SMALL
    ...
    # Group templates by zone; process only SVNS_ACTIVE_ZONES in fixed order.
    ...
    def _svns_brand_pass_order(brand: Optional[str]) -> Tuple[int, str]:
        b = (brand or "").lower()
        if b in DIFFICULT_MATCH_BRANDS:
            return (0, b)
        if b in TIRE_BRANDS:
    ...
    for zone_name in SVNS_ACTIVE_ZONES:
        ...
        if not zone_templates:
            continue
        brand_key = SVNS_BRAND_NAME.lower()
        zone_templates = [
            t for t in zone_templates if t[2] and t[2].lower() == brand_key
        ]
        zone_templates = sorted(zone_templates, key=lambda t: _svns_brand_pass_order(t[2]))
...
def visualize_zones(
    ...
    if zones_to_show is None:
        zones_to_show = list(SVNS_ACTIVE_ZONES)
...
def create_json_output(
    ...
    json_output = {
        ...
        "prefs": {
            ...
            "prefsFilename": "svns_templates.txt",
...
def find_svns_folders(root_dir: Path) -> list[Path]:
    """Find subfolders whose name starts with ``svns`` (case-insensitive)."""
    folders: list[Path] = []
    for path in root_dir.rglob("*"):
        if path.is_dir() and path.name.lower().startswith("svns"):
            folders.append(path)
    return sorted(set(folders))


def find_non_median_reduced_folders(root_dir: Path) -> list[Path]:
    """Find ``svns*`` folders under root (same as batch; kept for single-root batch layout)."""
    return find_svns_folders(root_dir)
...
def process_single_folder(image_folder: Path, args, templates, brand_mapping, script_dir, output_dir) -> int:
    ...
    def process_single_image(image_path: Path, visualize_zones_flag: bool = False) -> Tuple[Path, Dict[str, Any], List, int]:
        ...
        try:
            ...
            detections = detect_svns_in_region(
            ...
            if args.annotate:
                ...
                if preview_thresh > 0 and preview_thresh < args.threshold:
                    preview_detections = detect_svns_in_region(
                        image_path,
                        templates,
                        preview_thresh,
                        args.region_only,
            ...
            if detections:
                ...
                if detection_count == 0:
                    zones = sorted(LOADED_TEMPLATE_ZONES or SVNS_ACTIVE_ZONES)
                    print(
            ...
            if visualize_zones_flag:
                ...
                visualize_zones(
                    image_path, zone_viz_path, zones_to_show=list(SVNS_ACTIVE_ZONES)
                )
    ...
    print(f"📊 Summary: {total_detections} SVNS detection(s) in {len(images)} image(s)")
...
def main():
    parser = argparse.ArgumentParser(description="SVNS Template Matcher")
    parser.add_argument(
        "image_folder",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"{DEFAULT_THRESHOLD_SVNS_ONLY} with --SVNSonly)"
        ),
    )
    ...
    parser.add_argument("--templates-dir", type=Path, default=None,
                       help="Path to templates directory (default: ./svns_templates or ./svns_tempates)")
    parser.add_argument("--output-dir", type=Path, default=None,
                       help="Path to output directory (default: ./svns_output)")
    ...
    parser.add_argument("--workers", type=int, default=None,
                       help="Parallel workers: with --SVNSonly = folders in parallel (default 4); else = images per folder (default: auto)")
    ...
    parser.add_argument(
        "--SVNSonly", "--SVNSOnly", action="store_true", dest="SVNSonly",
        help="Batch mode: process all subfolders whose name starts with 'svns'",
    )
    ...
    if args.threshold is None:
        args.threshold = (
            DEFAULT_THRESHOLD_SVNS_ONLY if args.SVNSonly else DEFAULT_THRESHOLD
        )

    global SVNS_ACTIVE_ZONES
    SVNS_ACTIVE_ZONES = SVNS_ALL_ZONES

    # Setup paths
    script_dir = Path(__file__).parent
    templates_dir = resolve_svns_templates_dir(script_dir, args.templates_dir)
    output_dir = args.output_dir or (script_dir / "svns_output")
    ...
    svns_templates = [
        ...
        if t[4] in SVNS_ACTIVE_ZONES and t[2] and t[2].lower() == brand_key
    ]
    ignored = len(templates) - len(svns_templates)
    if ignored:
        print(
            f"⚠️  Ignored {ignored} template(s) "
            f"(zone={SVNS_MATCH_ZONE}, brand={SVNS_BRAND_NAME})"
        )
    templates = svns_templates
    ...
    LOADED_TEMPLATE_ZONES = frozenset(
        z for z in (t[4] for t in templates) if z in SVNS_ACTIVE_ZONES
    )
    
    if not templates:
        print(
            "❌ No templates found for active zone(s): "
            f"{', '.join(SVNS_ACTIVE_ZONES)}."
        )
        print(
            f"   Expected: {SVNS_TEMPLATES_DIR_NAMES[0]}/HSBC SVNS Logos/"
            f"{SVNS_MATCH_ZONE}/{SVNS_TOUCHPOINT}/*.jpg "
            f"(or svns_tempates/... if you use that folder name)"
        )
        return 1
    
    cw, ch = get_annotool_canvas_size(1920, 1080)
    zones_searched = sorted(LOADED_TEMPLATE_ZONES or SVNS_ACTIVE_ZONES)
    print(
        f"🎯 SVNS brand={SVNS_BRAND_NAME} touchpoint={SVNS_TOUCHPOINT} "
        f"zone={SVNS_MATCH_ZONE} "
        f"| zones with templates: {', '.join(zones_searched)} "
        f"| JSON canvas ~{cw}×{ch} (1080p)"
    )
    print(
        f"   conf≥{SVNS_MIN_ANNOTATION_CONFIDENCE} "
        f"| csv%≥{SVNS_MIN_CSV_PERCENT_BY_BRAND.get(brand_key, SVNS_MIN_CSV_PERCENT)}"
    )
    ...
    mapping_file = script_dir / "svns_brand_mapping.txt"
    brand_mapping = load_brand_mapping(mapping_file)
    if brand_mapping:
        print(f"📋 Loaded brand mapping (fallback): {brand_mapping}")
    else:
        print(f"ℹ️  No svns_brand_mapping.txt found - using folder structure for brand/location detection")
    
    # Check for batch processing mode (--SVNSonly)
    if args.SVNSonly:
        ...
        print(f"\n🔍 Batch mode: Searching for folders starting with 'svns' in {root_folder}")
        svns_folders = find_svns_folders(root_folder)
        
        if not svns_folders:
            print(f"❌ No folders starting with 'svns' found in {root_folder}")
            return 1
        
        print(f"📁 Found {len(svns_folders)} folder(s) to process:\n")
        for i, folder in enumerate(svns_folders, 1):
            print(f"   {i}. {folder}")
        print()
        
        # Number of folders to process in parallel (--workers applies to folder-level when SVNSonly)
        import os
        folder_workers = args.workers if args.workers is not None else min(4, len(svns_folders), os.cpu_count() or 4)
        folder_workers = max(1, min(folder_workers, len(svns_folders)))
        print(f"🚀 Using {folder_workers} parallel worker(s) for folders.\n")
        
        total_folders = len(svns_folders)
        failed_folders = []
    ...
    # Check if it's a root directory (contains subfolders with "reduced_" that don't end in "_svns_batch")
    ...
    if non_median_folders:
        ...
        print(f"\n🔍 Batch mode: Searching for 'reduced_*' folders (excluding '_svns_batch') in {image_folder}")
    ...
    # Without --SVNSonly, do not process folders ending with _svns_batch (use --SVNSonly for those)
    if image_folder.is_dir() and image_folder.name.endswith("_svns_batch"):
        print(f"❌ Without --SVNSonly, folders ending with '_svns_batch' are not processed. Use --SVNSonly to process median folders.")
        return 1
