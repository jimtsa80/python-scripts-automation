...
def main(images_folder, excel_file):
    ...
    # Define the UNC path
    unc_path = r"\\?\\" + images_folder

    # Extract the last part of the folder name
    last_part = os.path.basename(images_folder)

    # Create the new folder using the UNC path
    new_folder = os.path.join(unc_path, "reduced_" + last_part)
    ...
    # Create Excel file for sequences with filenames in the second column
    sequence_data = []
    for sequence_folder in sequence_folders:
        sequence_images = os.listdir(sequence_folder)
        first_image = sequence_images[0]
        image_filenames = ', '.join(sequence_images[1:])
        sequence_data.append([first_image, image_filenames])

    sequence_df = pd.DataFrame(sequence_data, columns=['First Image', 'Image Filenames'])

    # Save the Excel file one folder outside the new_folder
    parent_folder = os.path.dirname(new_folder)
    sequence_excel_path = os.path.join(parent_folder, "sequences_info_plus.xlsx")
