...
import tempfile
import re
import shutil  # Import shutil for cross-drive file moving
    ...
    with zipfile.ZipFile(input_zip, 'r') as in_zip:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for part_num in range(1, num_parts + 1):
                ...
                final_output_zip = os.path.join(output_dir, f"part{part_num}_{base_filename}.zip")
                shutil.move(output_zip, final_output_zip)  # Use shutil.move for cross-drive compatibility

                print_first_and_last_jpeg(final_output_zip)

    # Remove the initial zip file
    os.remove(input_zip)
    print(f"Removed the initial zip file: {input_zip}")
