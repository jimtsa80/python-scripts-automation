...
# Resolve helper paths once
$script_dir = Split-Path -Parent $MyInvocation.MyCommand.Path
$repo_root = Split-Path -Parent $script_dir
$fast_ad_remover_clip = Join-Path $repo_root "imagesClassification\fast_ad_remover_clip.py"
...
Get-ChildItem -Path $parent_dir -Directory | ForEach-Object {
    ...
    if ($clusters -ne "") {
        ...
        if ($reduced_folder) {
            ...
            # Remove ads BEFORE running any image classification on reduced folder
            if (Test-Path $fast_ad_remover_clip) {
                Write-Host "Running fast_ad_remover_clip.py on: $reduced_folder_fullpath"
                try {
                    python "$fast_ad_remover_clip" "$reduced_folder_fullpath" 0.5
                    # Small delay to ensure file moves are completed
                    Start-Sleep -Milliseconds 500
                } catch {
                    Write-Host "fast_ad_remover_clip.py failed: $($_.Exception.Message)" -ForegroundColor Red
                }
            } else {
                Write-Host "Warning: fast_ad_remover_clip.py not found at $fast_ad_remover_clip" -ForegroundColor Yellow
            }
            ...
            # $script_dir = Split-Path -Parent $MyInvocation.MyCommand.Path
            # $brands_file = Join-Path $script_dir "OCR_brands.txt"
