    ...
    for base_filename, part_files in part_files_dict.items():
        ...
        if dfs:
            ...
            try:
                ...
                print(f"Warning: Could not sort {base_filename} by column '{sort_column}' due to type error: {e}")
                # Optional: Uncomment the following lines to force string-based sorting
                # combined_df[sort_column] = combined_df[sort_column].astype(str)
                # combined_df.sort_values(by=sort_column, inplace=True)
