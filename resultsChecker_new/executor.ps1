...
# Define the Python script (folder, sheet_name)
$pythonScript = ".\importerResults_new.py"
...
Get-ChildItem -Path $sourceFolder -Filter "*.xlsx" | ForEach-Object {
    ...
    # Execute the Python script with $targetFolder as folder and sheet_name
    Write-Output "Running Python script on $($file.Name)..."
    python $pythonScript $targetFolder $targetFolder
}
