...
def create_brand_location_txt(sheet_name):
    ...
    # Print summary of unique brands and locations
    unique_brands_count = len(sorted_brands)
    unique_locations = set()
    for locations in brand_location_dict.values():
    ...
    print(f"Unique brands: {unique_brands_count}")
    print(f"Unique locations: {unique_locations_count}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process Google Sheet to create Brand/Location text file.")
    parser.add_argument("sheet_tab_name", help="The exact name of the Google Sheet tab (e.g. 'WRC')")

    args = parser.parse_args()

    # Use the sheet tab name exactly as provided (no 'Total_' prefix)
    create_brand_location_txt(args.sheet_tab_name)
